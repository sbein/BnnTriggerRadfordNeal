...
class NetWriter:
    ...
    def __init__(self, name, numnet, ninputs, nhidden, model, writeHeader):
        ...
        out.write('// Code Authors: Sam Bein, Harrison Prosper, Sezen Sekmen\n')
        out.write('// Based on CMS Internal Note: IN2011_019 by Prosper, Sekmen, Don Teo')
        out.write('// Inputs:\n')
    ...
    def close(self):
        ...
        out.write('\n')
        str = 'std::vector<double> Eff_%s_CenterUpDown(std::vector<double>& inputs,\n' % name
        tab = (find(str, "(") + 1)*" "
        out.write(str)
        ...
        out.write('  std::vector<double> v;\n')
        out.write('  for(int i=first; i <= last; i++) v.push_back(netfun(in, weight[i]));\n')
        out.write('  std::sort(v.begin(), v.end());\n')
        out.write('  double median = v[int(nnetworks/2)];\n')
        out.write('  double up = 0.5*(v[int(0.84135*nnetworks)]+v[int(0.84135*nnetworks)+1]);\n')
        out.write('  double down = 0.5*(v[int(0.15865*nnetworks)]+v[int(0.15865*nnetworks)+1]);\n')
        out.write('  std::vector<double> CenterUpDown;\n')
        out.write('  CenterUpDown.push_back(median);\n')
        out.write('  CenterUpDown.push_back(up);\n')
        out.write('  CenterUpDown.push_back(down);\n')
        out.write('  return CenterUpDown;\n')
        out.write('  \n')
        ...
        if len(var) < MAXARGS:
            ...
            out.write('\n')		
            str = 'std::vector<double> Eff_%s_CenterUpDown(' % name
            space = (find(str,'(')+1)*' '
            tab = ''
            out.write(str)
            ...
            for index, (namen, mean, sigma) in enumerate(var):
                ...
                out.write('  in[%d] = %s;\n' % \
                          (index, namen))
            out.write('  return Eff_%s_CenterUpDown(in, first, last);\n}\n' % name)
        else:
